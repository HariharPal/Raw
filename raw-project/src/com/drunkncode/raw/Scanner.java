...
class Scanner {
    ...
    // for each turn of loop we scan a single token. It provides the lexers the
    // token type in which single charactor imagined as lexeme. Just we need to
    // consume the next charactor and pick a token type for it.
    // if there's any unexpected character in the code then we try to throw error
    // becuase after an error the hadError goes true but we still throw all error in
    // one pass becuase we don't want to wait to error shows up after fixing one to
    // surprise us.
    private void scanToken() {
        ...
        switch (c) {
            ...
            // if that's an digit we check if it's boolean or double means if it's floating
            // point number we use to say token as NUMBER(...) or if that's DOUBLE(...)
    ...
    private void number() {
        ...
        // we add that number to the token and we pass literal as substring of start
        // till current ;
    ...
    private void string() {
        // Either two possiblity eiter we encounter the end of the source file or " end
        // of string token
        // we proceed and traverse till we get end or " terminal quotes and if we
        // encounter \n we increaese line;
        while (peek() != '"' && !isAtEnd()) {
            if (peek() == '\n') {
                line++;
    ...
    // it's called lookahead it does'nt like advance that consumes the charactor it
    // peeks the character looks for current consumed charactor;
    private char peek() {
        if (isAtEnd()) {
            return '\0';
        }
    ...
    // used to recognize the next charactor we want to expect if thats true then we
    // return true and increment our currrent pointer to next charactor but if it's
    // not equal then we return false;
    // match is used in scanToken so that advance plays as it is but match tries to
    // match next charactor for some special operators ;
    ...
    // consumes the next charactor of the source file whenever the scan token is
    // called and because scan token is being called till source file is at end the
    // entire source file must converted into the List of tokens ;
